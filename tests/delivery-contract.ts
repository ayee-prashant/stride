...
export async function setupDelivery(makeFixture: () => Promise<DeliveryFixture>) {
  const f = await makeFixture(); const s = new DeliveryService(f.repo); const c = new AgentConnections(f.repo); const roles = new AgentRegistryRepository(f.repo); const user = f.owner.userId;
...
export async function completeDelivery(makeFixture: () => Promise<DeliveryFixture>) {
  ...
  async function accept(ticket: DeliveryTicket, _result: AgentReport) { return (await f.s.review(user, f.workspace, f.project, ticket.id, { request_id: rid(), expected_version: ticket.version, decision: "accept", report_hash: digest(ticket.payload.report), reason: "Human independently reviewed the submitted outcome" })).ticket!; }
