...
| AW-002 | Test client transport and permission capabilities | Versioned matrix for Codex, Claude, Gemini and Antigravity; record supported, failed and untested features separately |
...
Demo: create a ticket, give DEV-1 one contribution, receive a notice, approve it, ask the agent to work, inspect its linked change/report and accept it. Repeat with a disconnect, rejected permission and requested revision. Keep a human reviewer fallback; no second agent is required for first usefulness.
...
Measure human effort and delivery quality: time from assignment to a reviewed start, time waiting for a named human, accepted outcomes, rework and escaped defects, duplicate launches, stale-evidence incidents and failure recovery. Compare with the team's baseline before setting improvement claims. More agent activity alone is not product success.
