...
Use remote HTTPS Streamable HTTP as the preferred MCP transport. Supply a trusted local stdio bridge for clients that cannot complete the remote auth flow. The bridge uses separately enrolled credentials from protected local storage and never emits diagnostics on its protocol stdout. The research baseline is the [2026-07-28 transport specification](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports).

Use standard modern MCP subscriptions for context/work changes when supported. The companion can subscribe and present IDE/OS notices; it remains responsible for execution monitoring. Supply an authenticated application catch-up API and optional SSE/polling fallback for other clients and the human UI. Do not depend on sampling, an open chat, keyboard injection or undocumented extension APIs to start arbitrary idle agents. These channels project one durable outbox.

Implement the modern per-request version model through a maintained SDK and publish the tested client/version set. Older handshake-based clients need an explicit compatibility adapter. Version Stride payloads and context manifests separately, apply strict schemas and content/rate limits, and reject unsupported contracts without silently weakening validation. [Context protocol](CONTEXT_PROTOCOL.md) records the modern subscription, cancellation, metadata and recovery requirements.
...
Use OAuth authorization code with PKCE for compatible remote MCP clients, protected-resource metadata and audience-bound access tokens. Pre-register the initial client set or use validated client metadata under the current registration rules. Verify issuer handling, redirects, fetch restrictions and abuse controls; reserve legacy dynamic registration for tested compatibility needs. Native app clients cannot keep a confidential client secret. Never forward the Stride token as a GitHub or model-provider token. [Current MCP authorization](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization).
...
Context read/search/validation, acknowledgement, proposals and checkpoints are defined in [Context protocol](CONTEXT_PROTOCOL.md). New ticket/plan proposals include requirement links, current context and evidence of the gap. Active writes also require the current material context binding. MCP request metadata and any legacy protocol session are not persisted agent identities.
...
Conformance cases: connect/revoke, profile isolation, manual start, duplicate launch request, stale authorization/context, packet loading, successful submission, tool denial, human clarification, process failure, disconnect/reconnect, cancellation and usage marked unavailable. Also test modern metadata/subscriptions, incompatible versions and required MCP connection failure, including initialization for a supported legacy client. Record exact binary/SDK version and evidence before enabling assisted launch for that combination. The vendor table above is not proof that those clients support the new protocol revision.
