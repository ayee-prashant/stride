...
export function DeliveryCenter({ workspaceId, projectId, members, userId, admin }: { workspaceId: string; projectId: string; members: Member[]; userId: string; admin: boolean }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!c.signal.aborted) { setPage(overview); setBindings(roles); setNotices(notificationPage.notices); setError(""); }
  ...
  }, [path, offset, projectId, workspaceId]);
  useEffect(() => { void refresh(); const focus = () => { void refresh(); }; window.addEventListener("focus", focus); const timer = window.setInterval(() => { if (!document.hidden && !selected) void refresh(); }, 30000); return () => { abort.current?.abort(); clearInterval(timer); window.removeEventListener("focus", focus); }; }, [refresh, selected]);
