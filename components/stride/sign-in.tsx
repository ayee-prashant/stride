...
export function SignIn({ available }: { available: boolean }) {
  ...
  return <main className="flex min-h-svh items-center justify-center bg-background p-6">
    <section className="w-full max-w-sm space-y-6 rounded-2xl border bg-card p-8 shadow-sm">
      ...
      {available ? <Button className="w-full" disabled={busy} onClick={signIn}><LogIn aria-hidden="true" />{busy ? "Opening GitHub…" : "Continue with GitHub"}</Button> : <p className="text-sm text-muted-foreground" role="status">Your workspace is being prepared. Sign-in will be available when setup is complete.</p>}
