...
import type { Activity, Member, Task, TaskPatch } from "@/lib/domain";
...
export function TaskEditor({ task, members, onClose, onUpdate }: { task: Task; members: Member[]; onClose: () => void; onUpdate: (task: Task, patch: Omit<TaskPatch, "version">) => Promise<Task> }) {
  ...
  const dirty = JSON.stringify(draft) !== JSON.stringify(editable(source));
  useEffect(() => {
  ...
  }, [source.id, source.workspace_id, source.version]);
      ...
      <form className="editor-form" onSubmit={event => { event.preventDefault(); void save(draft); }}>
        <fieldset disabled={busy || !!source.archived_at} className="editor-fields">
          ...
          <div className="field-grid">
            ...
            <div className="field"><span className="field-label">Assignee</span><Choice label="Task assignee" value={draft.assignee_id ?? "unassigned"} onChange={value => setDraft({ ...draft, assignee_id: value === "unassigned" ? null : value })} options={[{ value: "unassigned", label: "Unassigned" }, ...members.map(m => ({ value: m.user_id, label: m.name }))]} /></div>
