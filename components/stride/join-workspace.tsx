...
export function JoinWorkspace({ signedInEmail }: { signedInEmail: string | null }) {
  ...
  const [name, setName] = useState(""); const [password, setPassword] = useState(""); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [joined, setJoined] = useState(false); const [destination, setDestination] = useState("/");
  useEffect(() => {
  ...
  return <main id="main-content" className="auth-page"><section className="auth-card"><p className="font-semibold text-primary">Stride</p><h1>{joined ? "You’re ready to join" : "Join your team"}</h1>
    {joined ? <><p>Your account and membership are ready. Sign in with your password to open the workspace.</p><a className="auth-link" href={`/sign-in?${new URLSearchParams({ next: destination })}`}>Continue to sign in</a></> : preview ? <><p>You’ve been invited to <strong>{preview.workspace_name}</strong> as <strong>{preview.email}</strong>.</p>
