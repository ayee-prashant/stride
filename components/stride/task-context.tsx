...
import { RepositoryObservation } from "./repository-context";
...
export function TaskContext({ task, disabled, onBusyChange, onDirtyChange }: { task: Task; disabled: boolean; onBusyChange: (busy: boolean) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  useEffect(() => {
    if (!open) return;
    void refresh();
    const focus = () => { if (!lock.current && document.visibilityState === "visible") void refresh(); };
    const interval = window.setInterval(focus, 20000);
    window.addEventListener("focus", focus);
    return () => { controller.current?.abort(); window.clearInterval(interval); window.removeEventListener("focus", focus); };
  }, [open, refresh, task.version]);
  ...
  return <section className="task-context-section"><button type="button" className="task-context-toggle" aria-expanded={open} onClick={() => setOpen(!open)}><BookOpen size={17} /><span>Task brief</span><span>{open ? "Hide" : "Review context"}</span></button>{open && <div className="task-context-content">
    ...
    {!data && !error ? <p role="status">Loading task context…</p> : data && <>
      {data.result.check && <div className={`task-brief-status brief-${data.result.check?.state}`}><strong>{data.result.check?.state === "current" ? "Brief is current" : data.result.check?.state === "stale" ? "Brief needs a review" : "Brief is unavailable for work"}</strong>{data.result.check?.reasons.map(reason => <p key={reason}>{reason}</p>)}</div>}
      ...
      {data.result.brief && <details className="saved-task-brief"><summary>Read the saved brief</summary><h4>{data.result.brief.payload.task.title}</h4><p className="brief-body">{data.result.brief.payload.task.description || "No additional task description."}</p>{data.result.brief.payload.documents.map(doc => <section key={doc.document_id}><h4>{doc.title} <span className="muted">v{doc.version} · {doc.kind}</span></h4><p className="brief-body">{doc.body}</p></section>)}{data.result.brief.payload.repository ? <section><h4>Repository snapshot</h4><RepositoryObservation observation={data.result.brief.payload.repository.observation} /></section> : <p className="muted text-sm">No GitHub source was connected when this brief was prepared.</p>}<p className="muted text-sm">Prepared {new Date(data.result.brief.created_at).toLocaleString()}. This saved copy is preserved when requirements change.</p></details>}
    </>}
    <p className="brief-integration-note">Connected GitHub sources must be verified before preparing a brief. Agent start approvals are still pending; this brief does not authorize execution.</p>
  </div>}</section>;
}
