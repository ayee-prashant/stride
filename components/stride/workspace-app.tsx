...
import { Archive, ArrowRight, BookOpen, Check, CheckCheck, Circle, CircleDashed, Folder, FolderKanban, LayoutList, ListTodo, Loader2, KeyRound, LogOut, Pencil, Play, Plus, RefreshCw, RotateCcw, Search, ShieldCheck, Users } from "lucide-react";
...
type View = "start" | "my" | "board" | "projects" | "context" | "agents" | "delivery" | "members";
...
const headings: Record<View, string> = { start: "Getting started", my: "My Tasks", board: "Project board", projects: "Projects", context: "Project brief", agents: "Team and agents", delivery: "Agent delivery", members: "People" };
...
export function WorkspaceApp({ identity, initialLink = { workspaceId: "", taskId: "" } }: { identity: Identity; initialLink?: { workspaceId: string; taskId: string; projectId?: string; deliveryId?: string } }) {
 ...
 const [projectDialog, setProjectDialog] = useState<Project | "new" | null>(null);
  ...
  return <SidebarProvider><Sidebar className="stride-sidebar"><SidebarHeader><div className="brand"><span className="brand-mark"><CheckCheck size={23} /></span><span>stride<span className="brand-period">.</span></span></div>{workspaces.length > 1 ? <Choice label="Workspace" value={workspaceId} onChange={changeWorkspace} options={workspaces.map(w => ({ value: w.id, label: w.name }))} /> : <p className="workspace-label">{workspace?.name ?? "Your workspace"}</p>}</SidebarHeader>
    <SidebarContent><SidebarGroup><SidebarGroupLabel>WORKSPACE</SidebarGroupLabel><SidebarMenu>{([{ view: "start", label: "Getting started", icon: BookOpen }, { view: "my", label: "My Tasks", icon: ListTodo }, { view: "board", label: "Project board", icon: FolderKanban }, { view: "projects", label: "Projects", icon: Folder }, { view: "context", label: "Project brief", icon: LayoutList }, { view: "agents", label: "Team and agents", icon: ShieldCheck }, { view: "delivery", label: "Agent delivery", icon: ShieldCheck }, { view: "members", label: "People", icon: Users }] as const).map(item => <SidebarMenuItem key={item.view}><SidebarMenuButton isActive={view === item.view} onClick={() => navigate(item.view)} className="nav-item"><item.icon /><span>{item.label}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup>
