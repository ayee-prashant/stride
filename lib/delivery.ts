...
export type PlanItem = { key: string; title: string; description: string; acceptance: string[]; todo: string[]; requirement_ids: string[]; read_paths: string[]; write_paths: string[]; depends_on: string[] };
...
export function covers(path: string, allowed: string[]) { return allowed.some(p => path === p || path.startsWith(p + "/")); }
...
export function parsePlanItem(input: unknown): PlanItem {
  const v = object(input, ["key", "title", "description", "acceptance", "todo", "requirement_ids", "read_paths", "write_paths", "depends_on"]);
  ...
  return { key: identifier(v.key), title: text(v.title, "Title", 200), description: text(v.description, "Description", 6000), acceptance, todo, requirement_ids: requirements, read_paths: reads, write_paths: writes, depends_on: listOfText(v.depends_on, "Dependencies", 25, 160).map(identifier) };
}
