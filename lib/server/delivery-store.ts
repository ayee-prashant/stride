...
export class DeliveryStore {
  ...
  async sessionActive(sessionId: string | null, operatorId: string) { return !!sessionId && !!this.options.sessionActive && await this.options.sessionActive(sessionId, operatorId); }
  ...
  async replay(actor: DeliveryActor, workspaceId: string, projectId: string, requestId: string, inputHash: string) {
    ...
    if (!e) return null;
    if (e.actor_kind !== actor.kind || e.actor_id !== (actor.kind === "human" ? actor.id : actor.profile_id) || e.input_hash !== inputHash || actor.kind === "agent" && (JSON.parse(e.payload).connection_id !== actor.connection_id || JSON.parse(e.payload).session_fingerprint !== digest(actor.session_id))) throw changed("This request identifier was already used for a different action.");
