...
export class DeliveryRepository extends DeliveryStore {
  ...
  async snapshot(userId: string, t: DeliveryTicket, bindingId: string): Promise<WorkPacket> {
    ...
    return { review_mode: !t.payload.candidate_profile || t.role_id === "development" ? "not_applicable" : t.payload.candidate_profile === b.profile_id ? "self_review" : "separate_profile", format: "stride-work-packet/1", ticket_id: t.id, ticket_version: t.version + 1, kind: t.kind, title: task.title, role_id: b.role_id, binding_id: b.id, binding_version: b.version, template_hash: b.template_hash,
