...
export async function deliveryRoute(repo: Repository, userId: string, workspaceId: string, url: URL, method: string, input: unknown, options: DeliveryOptions = {}, provision?: ConnectionProvisioner) {
  ...
  if (!m) return null;
  for (const key of url.searchParams.keys()) if (!["workspace_id", "offset", "after"].includes(key) || url.searchParams.getAll(key).length !== 1) throw new AppError(400, "INVALID_INPUT", "Unknown or repeated delivery query parameter.");
  ...
  else if (section === "notices" && !id && method === "GET") body = await s.notices(userId, workspaceId, projectId, parseContextCursor(url.searchParams.get("after")));
