...
try { while (running) { try { await new DeliveryService(repo, { bindings }).reconcile(); if (provider) for (let i = 0; i < 5 && running && await evidence.process(provider); i++) { /* bounded worker batch */ } } catch { console.error(JSON.stringify({ event: "agent_delivery_worker_retry" })); } if (process.argv.includes("--once")) break; if (running) await new Promise(resolve => setTimeout(resolve, 10000)); } } finally { await pool.end(); }
